...
class AirCargoProblem(Problem):
    ...
    def result(self, state: str, action: Action):
        ...
        pos_list = list(set(fs.pos + action.effect_add) - set(action.effect_rem))
        neg_list = list(set(fs.neg + action.effect_rem) - set(action.effect_add))
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        # Just count the cargo at needs to be moved to goal
        count = len(set(self.goal) - set(fs.pos))
        return count
...
def air_cargo_p1() -> AirCargoProblem:
    ...
    neg = init_neg_list(airports, cargos, planes)
...
def init_neg_list(airports, cargos, planes):
    neg = []
    for p in planes:
        for c in cargos:
